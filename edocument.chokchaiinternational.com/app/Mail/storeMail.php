...
class storeMail extends Mailable
{
    ...
    {     
        ...
        return $this->subject('เอกสารขอการอนุมัติ ประจำวันที่ '.date('Y/m/d'))
        ->view('email.Mail', compact('data'));
    }
    ...
    public function Query_document($id)
    {
        ...
        'users.name as name', 'document_receivers.email as email', 'document_receivers.position as position', 'document_receivers.passwrod as passwrod',
        'users.liff_usersid as users_liffID', 'sender.name as senderName')
        ...
        if(!empty($data->users_liffID)){
            $toppic=""; $liffUsersid=null;  $name=""; $detail=""; $email="";
            $liffUsersid=$data->users_liffID;
            $toppic= $data->document_title; 
            $detail=$data->document_detail;
            $name=$data->name;
